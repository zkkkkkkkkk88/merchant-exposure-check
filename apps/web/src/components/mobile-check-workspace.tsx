...
import { createMobileValidationSet, discoverMobileSourcesAction, saveMobileRound, selectMobileValidationQuestions } from "@/app/mobile-checks/actions";
import type { MobileSourceDiscoveryData, MobileValidationSetData, MobileWorkspaceData, QueryData } from "@/lib/contracts";
...
import { MobileSourceReview } from "./mobile-source-review";
...
export function MobileCheckWorkspace({ candidates, merchantId, merchantName, validationSet, workspace }: { candidates: QueryData[]; merchantId: string; merchantName: string; validationSet: MobileValidationSetData | null; workspace: MobileWorkspaceData }) {
  ...
  const [sourceDiscovery, setSourceDiscovery] = useState<MobileSourceDiscoveryData | null>(null);
  ...
  const discoverSources = async () => {
    ...
    setSourceError(null);
    try {
      const result = await discoverMobileSourcesAction({
        merchantId,
        payload: {
          location_text: location.trim() || null,
          competitors: countRecurringCompetitors(drafts, merchantName),
        },
      });
      setSourceDiscovery(result);
    } catch (error) {
      setSourceError(error instanceof Error ? error.message : "自动查找公开来源失败，请稍后重试。");
    } finally {
  ...
  return <div className="mobile-check-layout">
    {workspace.metrics && <section className="previous-results"><header><div><p className="kicker">PREVIOUS RESULT</p><h2>上一轮有效结果</h2></div><span>已确认 {workspace.metrics.confirmedCount} 道</span></header><div className="mobile-metrics" aria-label="手机版实测指标"><article><span>提及率</span><strong>{percent(workspace.metrics.mentionRate)}</strong><small>{workspace.metrics.mentionCount}/{workspace.metrics.confirmedCount}</small></article><article><span>首批推荐率</span><strong>{percent(workspace.metrics.primaryRate)}</strong><small>{workspace.metrics.primaryCount}/{workspace.metrics.confirmedCount}</small></article><article><span>场景覆盖率</span><strong>{percent(workspace.metrics.categoryCoverageRate)}</strong><small>{workspace.metrics.categoryCoveredCount}/{workspace.metrics.categoryTotalCount}</small></article><article><span>信息准确率</span><strong>{percent(workspace.metrics.informationAccuracyRate)}</strong><small>{workspace.metrics.informationAccurateCount}/{workspace.metrics.informationEvaluatedCount}</small></article></div><article className={`primary-outcome ${workspace.metrics.primaryCount > 0 ? "achieved" : "not-achieved"}`}><div><span>本轮至少首批推荐一次</span><small>交付目标：3个独立对话中至少1次进入首批推荐</small></div><strong>{workspace.metrics.primaryCount > 0 ? "是" : "否"}</strong></article></section>}
    ...
    {workspace.latestRoundId && !entryOpen && <section className="workspace-card round-complete">
      ...
      <button className="button primary" data-requires-admin="true" type="button" onClick={() => setEntryOpen(true)}>开始新一轮</button>
    </section>}
    {validationSet && entryOpen && <form className="workspace-card mobile-entry" action={saveMobileRound} data-requires-admin="true">
      ...
      <label>测试位置<input name="location" placeholder="例如：澜沧拉祜族自治县" value={location} onChange={(event) => setLocation(event.target.value)} /></label><label className="check-row"><input defaultChecked name="webSearch" type="checkbox" />已开启联网搜索</label>
      <label>集中粘贴3份回答<textarea aria-label="集中粘贴3份回答" name="rawQaText" rows={12} value={rawText} onChange={(event) => { setRawText(event.target.value); setParsed(false); setSourceDiscovery(null); setSourceError(null); }} placeholder={'请按下面格式粘贴：\nQ1：第一份完整回答\n\nQ2：第二份完整回答\n\nQ3：第三份完整回答'} /></label>
      ...
      {parsed && <MobileSourceReview error={sourceError} groups={sourceDiscovery?.groups ?? []} hasSearched={sourceDiscovery !== null} loading={sourceLoading} onDiscover={discoverSources} />}{workspace.sourceRoundId && <label className="check-row"><input name="inheritSources" type="checkbox" />本轮来源无变化，沿用上一轮</label>}
      ...
      <button className="button primary" data-requires-admin="true" type={parsed && !drafts.some((draft) => draft.needsReview) ? "submit" : "button"} onClick={parsed && !drafts.some((draft) => draft.needsReview) ? undefined : () => setParsed(true)}>{parsed && !drafts.some((draft) => draft.needsReview) ? "统一保存并确认本轮" : parsed ? "重新识别回答" : "识别回答并继续"}</button>
    </form>}
