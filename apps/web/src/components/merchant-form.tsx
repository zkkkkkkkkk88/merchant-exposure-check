...
export function MerchantForm({ onSubmit }: { onSubmit: (payload: MerchantPayload) => Promise<void> }) {
  ...
  return <form className="editorial-form" onSubmit={submit} noValidate>
    ...
    <fieldset><legend>公开来源</legend><label>公开来源 1<input name="source" type="url" placeholder="https://" /></label><p>可填写官网、门店页或公开报道，后续检测会保留来源。</p></fieldset>
    {error && <p className="form-error" role="alert">{error}</p>}
    <div className="form-actions"><button className="button primary" disabled={saving} type="submit">{saving ? "保存中…" : "保存并生成问题"}</button></div>
  </form>;
}
