...
export default async function HistoryPage({ searchParams = Promise.resolve({}) }: { searchParams?: Promise<{ merchant?: string }> }) {
  ...
  return <AppShell><div className="workspace-page"><header className="page-header"><div><p className="kicker">HISTORY</p><h1>历史对比</h1><p>展示最近两次真实有效检测的成长指标变化。</p></div></header><HistoryComparison leftLabel={label(left.finished_at ?? left.created_at)} rightLabel={label(right.finished_at ?? right.created_at)} deltas={{ readinessScore: Number(history.deltas.readiness_score ?? 0), profileCompleteness: Number(history.deltas.profile_completeness ?? 0), publicVerifiability: Number(history.deltas.public_verifiability ?? 0), highIntentHitRate: Number(history.deltas.high_intent_hit_rate ?? 0), competitorGapClosure: Number(history.deltas.competitor_gap_closure ?? 0) }} /></div></AppShell>;
}
